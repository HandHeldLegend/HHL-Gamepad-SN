...
// GPIO for USB control function
#define USB_GPIO_HID_EN     GPIO_NUM_4
#define USB_GPIO_LINE_EN    GPIO_NUM_16

#define USB_GPIO_MASK ((1ULL<<USB_GPIO_HID_EN) | (1ULL<<USB_GPIO_LINE_EN))
...
void local_system_evt(hoja_system_event_t evt, uint8_t param)
{
    ...
    switch(evt)
    {
        ...
        case HEVT_API_INIT_OK:
        {
            ...
            {
                ESP_LOGI(TAG, "USB Program Mode Enable.");
                vTaskDelay(1000/portTICK_PERIOD_MS);
                // Hold USB Line low, which holds C2D low causing USB bootloader on EFM8UB1
                GPIO.out_w1tc = 1<<USB_GPIO_LINE_EN;
                // Set HID high which releases the 'reset' line for the EFM8UB1
                GPIO.out_w1ts = 1<<USB_GPIO_HID_EN;
                //Delay 200ms
                vTaskDelay(200/portTICK_PERIOD_MS);
                // Set USB Line inactive
                GPIO.out_w1ts = 1<<USB_GPIO_LINE_EN;
                //Delay 200ms
                vTaskDelay(200/portTICK_PERIOD_MS);
                // Set USB Line active
...
void app_main()
{
    ...
    GPIO.out_w1tc = USB_GPIO_MASK;
    io_conf.intr_type = GPIO_INTR_DISABLE;
    io_conf.pin_bit_mask = USB_GPIO_MASK;
    io_conf.mode = GPIO_MODE_OUTPUT;
    io_conf.pull_up_en = GPIO_PULLUP_ENABLE;
    gpio_config(&io_conf);
